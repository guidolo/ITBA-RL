...
class BaseAgent:
    def __init__(self, ENV, logdir_root='logs', n_experience_episodes=1, gamma=0.999, epochs=1, lr=0.001, hidden_layer_neurons=128, EPISODES=2000, eval_period=50, algorithm='REINFORCE', noise=1.0, gif_to_board=False, fps=50, batch_size=128, LOSS_CLIPPING=0.2, ENTROPY_LOSS=5e-4):
        ...
        self.epochs = epochs
        self.lr = lr
        self.logdir = self.get_log_name(ENV, algorithm, logdir_root)  #path donde se guarda los datos para tensorboard
        self.env = gym.make(ENV)  #genera el environment
        ...
        self.model_train, self.model_predict = self.get_policy_model(lr=lr,
        ...
        #state_space_samples = np.array([self.env.reset() for x in range(10000)])

        state_space_samples = []
        self.env.reset()
        for x in range(10000):
            semirandomspace = self.env.step(np.random.choice([0, 1, 2, 3]))
            #si es fin de episodio se reinicia
            if semirandomspace[2]:
                self.env.reset()
            else:
                state_space_samples.append(semirandomspace[0])
    ...
    def get_action():
        #to implement in the real agent
        pass 
    
    #Guarda las experiencias (acciones y rewards) de cada episodio
    def get_experience_episodes(self, return_ts=False):
        ...
        while exp_episodes < self.n_experience_episodes:
            ...
            if done:
                exp_episodes += 1  #sumo 1 a la cantidad de experiencias
